...
_TRANSIENT_AMAP_ERRORS = {
    "CUQPS_HAS_EXCEEDED_THE_LIMIT",
    "CUQPS_HAS_EXCEEDED_THE_LIMIT_QPS",
...
async def _get_amap_json(
    ...
    *,
    ...
    """Request AMap with bounded retries for quota bursts and timeouts."""
    ...
    for attempt in range(retry_count + 1):
        try:
            ...
            if payload.get("status") == "1":
                return payload
            info = str(payload.get("info", "unknown"))
            if info not in _TRANSIENT_AMAP_ERRORS:
                raise RuntimeError(f"AMap request error: {info}")
            last_error = RuntimeError(f"AMap transient error: {info}")
    ...
    if last_error is not None:
        raise last_error
    raise RuntimeError("AMap request failed without a response")
...
class AmapGeocodingProvider:
    ...
    async def resolve(
        ...
        *,
        ...
        place_payload = await _get_amap_json(
            self.place_url,
            params=place_params,
        ...
        if pois:
            ...
            geocode_payload = await _get_amap_json(
                self.base_url,
                params=geocode_params,
                timeout_seconds=self.timeout_seconds,
            )
            geocodes = geocode_payload.get("geocodes", [])
            if not geocodes:
                self._cache[cache_key] = None
                return None
            result = geocodes[0]
            source_type = "amap_geocode"
            reference = result.get("formatted_address") or address
