...
class LocationRepository:
    ...
    def register_runtime(
        ...
        if existing:
            if (
                (existing.longitude is None or existing.latitude is None)
                ...
                merged = existing.model_copy(
                    update={
                        ...
                        "aliases": list(
                            dict.fromkeys(
                                [*existing.aliases, *location.aliases]
                            )
                        ),
                        "source": location.source,
                    }
                )
                self._locations[existing.id] = merged
                for alias in {
                    merged.name,
                    merged.id,
                    *merged.aliases,
                    ...
                    key = _normalize_name(alias)
                    if key:
                        self._alias_index[f"{campus_id}:{key}"] = merged.id
                return merged
            return existing
