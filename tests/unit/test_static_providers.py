...
@pytest.mark.asyncio
async def test_hdu_named_venues_use_cached_area_route_when_offline():
    locations = LocationRepository(DATA_DIR / "locations.json")
    routes = StaticRouteProvider(DATA_DIR / "travel_times.json", locations)

    first = await routes.get_route(
        "library_floor_7_11",
        "sf_express",
    )
    second = await routes.get_route(
        "sf_express",
        "northwest_track",
    )

    assert first.origin_id == "library_floor_7_11"
    assert first.destination_id == "sf_express"
    assert first.duration_min == 13
    assert second.duration_min == 15
    assert first.source == DataSource.ESTIMATED
    assert "高德" in (first.warning or "")
