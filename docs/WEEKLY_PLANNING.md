...
- 目标—阶段—时间块分配与容量不足显式报告；
- 周分配按日结合课表、校历、开放规则、路线和天气落地；
- 完成事件幂等记录与 `daily_rollover`、`task_incomplete`、`new_task`、
  `fixed_event_changed`、`weather_changed`、`manual` 六类版本化重排；
- 完成/锁定/已开始块冻结，目标、阶段和分配的跨版本血缘；
- 24 个冻结规划场景的基线和消融评测。
...
重排使用词典序目标，而不是把硬约束和偏好混成一个可能互相抵消的总分：
...
```json
{
  ...
  "query": "周五前完成课程设计，共8小时，其中编码4小时、测试2小时、报告2小时。"
}
```
...
`POST /api/v1/weeks/{plan_id}/days/{date}/materialize?user_id=...`

接口把当天分配转换成稳定任务 ID，合并课表与校历，加载路线、场馆规则和
天气，再经日规划器与校验器处理。只有 `valid` 计划会持久化并回写
`daily_plan_id`；不可行时返回具体 issue，重复调用保持幂等。
...
请求可提供最新 `capacities` 或 `availability`、受影响分配 ID；当触发类型为
`new_task` 时可用 `additional_goals` 增量加入目标，不需要重传原有全部目标。
服务端拒绝从已经被新版本替代的旧计划继续分叉。

### 8.6 手动锁定与解锁（后续界面能力）

当前 schema 和重排器已经识别 `locked` 并保证不移动，但面向普通用户的
`PATCH /api/v1/weeks/{plan_id}/allocations/{allocation_id}` 操作界面尚未
作为本轮核心技术交付，不得在材料中写成已上线功能。后续可允许用户：
...
- 已完成任务冻结；
- 锁定任务不移动；
- 部分完成 60 分钟中的 20 分钟后，只滚动剩余 40 分钟并保持总量守恒；
- 过期未完成块可回收，真正进行中的块继续冻结；
- 新增硬截止任务可最小驱逐柔性旧块并把旧块安置到剩余容量；
- 重复完成事件幂等；
- 计划版本不会覆盖；
...
- 未完成任务滚入后续日期；
- 新增高优先级任务后保留原计划；
- 旧版本的事件、重排和每日落地均返回 409；
- 周首选时段被课程占用时，可移动到同日其他可行窗口；
- 两个并发每日落地请求只发布一份 Plan，故障注入时 Plan、PlanItems 和绑定
  全部回滚；
- 排程期间完成进度变化会触发可重试快照冲突，过期计划不落库；
- 完成事件与 V2 重排并发时使用 latest + 完整基线指纹 CAS，不静默丢进度；
- 每日落地与 V2 重排并发时，旧版本 Plan 不会在新版本产生后落库；
- 法定节假日有个人活动但无常规课程；
- 调休工作日没有学校通知时不臆造补课；
- 当前版本固定使用杭电下沙校区知识底座。
...
|---|---:|---:|
| 硬约束违反率 | 0% | 0/24 |
| 硬截止满足率 | ≥ 95%（客观可行样本） | 8/8，100% |
| 阶段依赖正确率 | 100% | 专项测试全部通过 |
| 规划场景可行完成率 | ≥ 95% | 24/24，100% |
| 不应受影响任务误移动率 | 0% | 0/22 |
| 原计划总体保留率 | ≥ 75% | 26/36，72.2% |
| 未完成事件幂等率 | 100% | 专项测试通过 |
| 跨学校规则串用率 | 0% | 现有回归为 0 |

总体保留率包含必须移动的冲突任务，因此当前 72.2% 不包装成“达到 75%”；
更直接的无关任务指标为 22/22 保留。完整数据见
`reports/planning_benchmark.md`，冻结输入哈希见
